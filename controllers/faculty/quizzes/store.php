...
if (!empty($errors)) {
    ...
    header('Location: ' . base_url('/faculty/subject/' . $subject_id . '/quizzes'));
    exit;
}
...
try {
    ...
    for ($i = 0; $i < count($questions); $i++) {
        $db->query(
            "INSERT INTO quiz_questions (quiz_id, question_text, choice_a, choice_b, choice_c, choice_d, correct_answer)
    ...
    $db->rollBack();
    $_SESSION['error'] = 'Failed to create Quiz: ' . $e->getMessage();
}
