...
$submission = $db->query(
    "SELECT acs.*, a.* FROM assignment_submissions acs
     LEFT JOIN assignments a ON acs.assignment_id = a.assignment_id
...
try {
    ...
    $_SESSION['success'] = 'Student assignment evaluated successfully!';
} catch (Exception $e) {
    $_SESSION['error'] = 'Failed to evaluate student assignment: ' . $e->getMessage();
}

header('Location: ' . base_url('/faculty/submissions/subject/' . $subject['subject_id'] . '/assignment/' . $submission['assignment_id']));
exit;
