...
if (!$subject_id || !$project_id) {
    ...
    $_SESSION['error'] = 'Missing project or subject information.';
...
try {
    ...
    $db->query(
        "INSERT INTO project_submissions (
            project_id, student_id, file_path, notes, is_checked
        ) VALUES (
            :project_id, :student_id, :file_path, :notes, 0
        )",
        [
    ...
    if ($temp_path && $file_path) {
        ...
        if (!move_uploaded_file($temp_path, $uploadPath)) {
            ...
            header('Location: ' . base_url("/student/subject/$subject_id/project/$project_id"));
            exit;
        }
    }
