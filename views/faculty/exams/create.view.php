...
<main class="flex-1 overflow-y-auto px-30 py-6">
    ...
    <form method="POST" action="<?= base_url('/faculty/subject/exam/store') ?>" enctype="multipart/form-data">
            ...
            <div class="border-b border-gray-900/10 pb-12">
                ...
                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-2">
                            <input required id="num_questions" type="number" name="num_questions" min="1" max="100" placeholder="e.g. 10"
                                class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 outline-gray-300 focus:outline-2 focus:outline-green-600 sm:text-sm/6" />
                        </div>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-2">
                            <input required id="passing_score" type="number" name="passing_score" min="1" placeholder="e.g. 6"
                                class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 outline-gray-300 focus:outline-2 focus:outline-green-600 sm:text-sm/6" />
                        </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <p class="mt-1 text-xs text-gray-500">Set when the exam automatically closes.</p>
                    </div>

                    <div class="sm:col-span-2">
                        <label for="duration" class="block text-sm/6 font-medium text-gray-900">Duration (minutes)</label>
                        <div class="mt-2">
                            <input required id="duration" type="number" name="duration" min="1" max="300" value="60" placeholder="e.g. 60"
                                class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 outline-gray-300 focus:outline-2 focus:outline-green-600 sm:text-sm/6" />
                        </div>
                        <p class="mt-1 text-xs text-gray-500">Students have this much time to finish the exam.</p>
                    </div>
                </div>

                <!-- ✅ Buttons for generating and uploading Excel -->
                <div class="mt-4 flex flex-col sm:flex-row gap-4 items-start">
                    ...
                    <!-- ✅ Upload Excel -->
                    <div>
                        <label for="exam_excel" class="block text-sm/6 font-medium text-gray-900">Upload Excel File (Optional)</label>
                        <input type="file" id="exam_excel" accept=".xlsx, .xls"
                        ...
                        <p class="mt-1 text-xs text-gray-500">Upload Excel file to auto-fill exam questions.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500">Download sample Excel format for exam upload.</p>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const excelInput = document.getElementById('exam_excel');
    ...
    downloadTemplateBtn.addEventListener('click', () => {
        const data = [
            ...
            ["What is 5 + 5?", "5", "10", "15", "20", "B"],
            ["What is the capital of Japan?", "Seoul", "Beijing", "Tokyo", "Bangkok", "C"]
        ];
        const ws = XLSX.utils.aoa_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Exam Format");
        XLSX.writeFile(wb, "exam_format.xlsx");
    });
    ...
    passingScoreInput.addEventListener('input', () => {
        ...
        if (!isNaN(numQuestions) && !isNaN(passingScore) && passingScore > numQuestions) {
            alert('Passing score cannot be higher than number of items.');
