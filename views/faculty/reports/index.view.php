            ...
            <select id="subject_id" name="subject_id" class="w-full appearance-none rounded-md bg-white py-1.5 pr-8 pl-3 text-base text-gray-900 outline-1 outline-gray-300 focus:outline-2 focus:outline-green-600 sm:text-sm/6" <?= empty($subjects) ? 'disabled' : 'required' ?>>
                ...
                <?php else : ?>
                    ...
                    <?php foreach ($subjects as $subject) : ?>
                        <option value="<?= htmlspecialchars($subject['subject_id']) ?>"><?= htmlspecialchars(ucfirst($subject['subject_name'])) ?></option>
                    <?php endforeach; ?>
                <?php endif; ?>
            </select>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        generate.addEventListener('click', async () => {
            ...
            generateReport(student.value, subject.value);
        });
        ...
        async function getStudents(subjectId) {
            ...
            try {
                ...
                const response = await fetch('<?= base_url('/faculty/reports/students') ?>?subject=' + subjectId);
                ...
                if (!data || students.length === 0) {
                    options = `<option selected disabled>No Student Available</option>`;
                } else {
                    options = `<option value="" selected disabled>Select a Student</option>`;
                    students.forEach(student => {

                        const firstName = student.first_name ?
                            student.first_name.charAt(0).toUpperCase() + student.first_name.slice(1).toLowerCase() :
                            "";
                        const lastName = student.last_name ?
                            student.last_name.charAt(0).toUpperCase() + student.last_name.slice(1).toLowerCase() :
                            "";
                    ...
                    ${lastName}, ${firstName} ${middleInitial} - ${student.student_number}
                </option>`;
                    });
                ...
                <label for="student_id" class="block text-sm font-medium text-gray-900 mb-3">Student</label>
                <select id="student_id" name="student_id"
                class="w-80  rounded-md border border-gray-300 bg-white text-gray-900 focus:outline-none focus:ring-1 focus:ring-green-500 sm:text-sm"
                ${students.length === 0 ? "disabled" : "required"}>
                ${options}
                </select>
                `;
        ...
        async function generateReport(studentId, subjectId) {
            try {
                const response = await fetch(
                    `<?= base_url('/faculty/reports/generate') ?>?student=${studentId}&subject=${subjectId}`
                );
                const data = await response.json();
