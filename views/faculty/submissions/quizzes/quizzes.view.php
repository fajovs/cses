...
<header class="bg-white shadow-sm px-30 flex-none">
    <div class="mx-auto max-w-7xl py-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
         ...
         <div>
            <a href="<?= base_url('/faculty/submissions/quizzes') ?>"
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php foreach ($quizzes as $quiz): ?>
                                <tr class="hover:bg-gray-25 transition-colors duration-150 searchRow"
                                    ...
                                    <td class="px-6 py-5 text-center"><?= htmlspecialchars($quiz['submitted_count']) ?></td>

                                    <td class="px-6 py-5 text-center"><?= date("F j, Y g:i A", strtotime($quiz['deadline'])) ?></td>
                                    <td class="px-6 py-5 text-center"><?= date("F j, Y g:i A", strtotime($quiz['created_at'])) ?></td>
                                    ...
                                    <td class="px-6 py-5 text-center flex justify-center gap-2">
                                        ...
                                        <a href="<?= base_url('/faculty/submissions/subject/' . $quiz['subject_id'] . '/quiz/' . $quiz['quiz_id']) ?>"
                            ...
                            <?php if (empty($quizzes)): ?>
                                <tr>
                                    <td colspan="7" class="px-6 py-5 text-center text-gray-500">No active quizzes found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
