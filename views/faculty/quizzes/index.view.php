...
<header class="bg-white shadow-sm px-30 flex-none">
    <div class="mx-auto max-w-7xl py-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex flex-col gap-4">
            <a href="<?= base_url('/faculty/subject/' . $subject['subject_id']) . '/quiz' ?>"
...
<main class="flex-1 overflow-y-auto px-30 py-6">
    ...
    <!-- Active Quizzes -->
    <div class="mb-8">
        <h2 class="text-lg font-semibold text-gray-900 mb-2">Active Quizzes</h2>
        <input type="text" placeholder="Search active quizzes..."
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php foreach ($activeQuizzes as $quiz): ?>
                                <tr class="hover:bg-gray-25 transition-colors duration-150 searchRow"
                                    data-search-value="<?= htmlspecialchars($quiz['title']) ?>"
                                    data-group="active">
                                    <td class="px-6 py-5">
                                    ...
                                    <td class="px-6 py-5 text-center flex gap-2">
                                        <form method="POST" action=<?= base_url('/faculty/subject/quiz/state') ?>>
                                        ...
                                        <a href="<?= base_url('/faculty/subject/' . $quiz['subject_id'] . '/quiz/' . $quiz['quiz_id']) ?>"
    ...
    <!-- Inactive Quizzes -->
    <div>
        <h2 class="text-lg font-semibold text-gray-900 mb-2">Inactive Quizzes</h2>
            ...
            <div class="p-6 text-gray-600 text-sm">
                <div class="max-h-[500px] overflow-y-auto">
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php foreach ($inactiveQuizzes as $quiz): ?>
                                <tr class="hover:bg-gray-25 transition-colors duration-150 searchRow"
                                    data-search-value="<?= htmlspecialchars($quiz['title']) ?>"
                                    data-group="inactive">
                                    <td class="px-6 py-5">
                            ...
                            <?php endforeach; ?>
                            <?php if (empty($inactiveQuizzes)): ?>
                                <tr>
                                    <td colspan="6" class="px-6 py-5 text-center text-gray-500">No inactive quizzes found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                ...
                <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 id="dialog-title" class="text-base font-semibold text-gray-900">Delete Quiz?</h3>
                            <p id="dialog-message" class="mt-2 text-sm text-gray-500">Are you sure you want to delete this quiz? This action cannot be undone.</p>
                        </div>
                    </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse">
                    <form id="delete-form" method="POST" action="<?= base_url('/faculty/quiz/delete') ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const deleteQuizIdInput = document.getElementById('delete-quiz-id');
        ...
        document.querySelectorAll('.delete-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                dialogMessage.textContent = `Are you sure you want to delete the quiz "${quizTitle}"? This action cannot be undone.`;
