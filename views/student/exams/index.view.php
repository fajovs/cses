...
<main class="flex-1 overflow-y-auto px-30 py-6">
    ...
    <div class="mb-8">
        <h2 class="text-lg font-semibold text-gray-900 mb-2">In Progress Exams</h2>
        <input type="text" placeholder="Search In Progress exams..."
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php foreach ($inProgressExams as $exam): ?>
                                <tr class="hover:bg-gray-25 transition-colors duration-150 searchRow" data-group="active" data-search-value="<?= htmlspecialchars($exam['title']) ?>">
                                    <td class="px-6 py-5"><?= htmlspecialchars($exam['title']) ?></td>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php foreach ($attemptedExams as $exam): ?>
                                <tr class="hover:bg-gray-25 transition-colors duration-150 searchRow" data-group="inactive" data-search-value="<?= htmlspecialchars($exam['title']) ?>">
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        <?php
                                        $remarks = htmlspecialchars($exam['remarks']);
                                        $colorClass = ($remarks === 'passed') ? 'text-green-600' : 'text-red-600';
                                        ?>
                                        <strong class="<?= $colorClass ?>"><?= ucfirst($remarks) ?></strong>
                                    </td>
                                    <td class="px-6 py-5 text-center flex gap-2">
                                        <button
                    ...
                    <div class="relative mt-6 flex-1 px-4 sm:px-6">
                        <div class="space-y-12">
                            <div class="border-b border-gray-900/10 pb-12 flex gap-4 flex-col">
                                ...
                                <p id="feedback" class="mt-1 text-sm/6 text-gray-600"></p>
                            </div>
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('[command="show-modal"][commandfor="drawer"]').forEach(button => {
            button.addEventListener('click', () => {
                ...
                document.getElementById('title').innerHTML = `<strong>Title :</strong> <br> ${exam.title}`;
                document.getElementById('rating').innerHTML = `<strong>Score :</strong> <br> <span class='text-green-700'>${exam.score}</span>`;
                document.getElementById('feedback').innerHTML = exam.feedback ? `<strong>Feedback :</strong> <br> ${exam.feedback}` : '<strong>Feedback :</strong>';
            });
        });
    });
</script>
