...
<header class="bg-white shadow-sm px-30 flex-none">
    <div class="mx-auto max-w-7xl py-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm/6 text-gray-600">Number of Items: <strong><?= htmlspecialchars($quiz['num_questions']) ?></strong></p>
            <p class="mt-1 text-sm/6 text-gray-600">Deadline: <?= htmlspecialchars(date('F j, Y g:i A', strtotime($quiz['deadline']))); ?></p>
        </div>
    </div>
</header>

<main class="flex-1 overflow-y-auto px-30 py-6">
    <form method="POST" action="<?= base_url('/student/quiz/submit') ?>">
