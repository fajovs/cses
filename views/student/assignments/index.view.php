...
<main class="flex-1 overflow-y-auto px-30 py-6">
                ...
                <div class="max-h-[500px] overflow-y-auto">
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php foreach ($notSubmittedAssignments as $assignment): ?>
                                <tr class="hover:bg-gray-25 transition-colors duration-150 searchRow" data-group="active" data-search-value="<?= htmlspecialchars($assignment['title']) ?>">
                                    <td class="px-6 py-5">
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        <a href="<?= base_url('/student/subject/' . $assignment['subject_id'] . '/assignment/' . $assignment['assignment_id']) ?>"
                            ...
                            <?php endforeach; ?>
                            <?php if (empty($notSubmittedAssignments)): ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-5 text-center text-gray-500">No active assignments found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
    ...
    <div>
        <h2 class="text-lg font-semibold text-gray-900 mb-2">Submitted assignments</h2>
        <input type="text" placeholder="Search submitted assignments..."
            class="searchInput w-full mb-4 px-4 py-3 text-sm bg-white border-0 rounded-lg shadow-sm ring-1 ring-gray-200 focus:ring-2 focus:ring-green-500 focus:outline-none transition-all"
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php foreach ($submittedAssignments as $assignment): ?>
                                <tr class="hover:bg-gray-25 transition-colors duration-150 searchRow" data-group="inactive" data-search-value="<?= htmlspecialchars($assignment['title']) ?>">
                                    <td class="px-6 py-5 text-sm font-medium text-gray-900"><?= htmlspecialchars($assignment['title']) ?></td>

                                    <td class="px-6 py-5 text-center"><?= date("F j, Y g:i A", strtotime($assignment['submitted_at'])) ?></td>
                                    <td class="px-6 py-5 text-center font-semibold">
                                        <?= htmlspecialchars($assignment['total_score'] ?? '—') ?>
                                    </td>

                                    <td class="px-6 py-5 text-center text-gray-600">
                                        <?php if ($assignment['is_checked']): ?>
                            ...
                            <?php if (empty($submittedAssignments)): ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-5 text-center text-gray-500">No submitted assignments found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
            ...
            <el-dialog-panel class="group/dialog-panel relative ml-auto block size-full max-w-2xl transform transition duration-500 ease-in-out data-closed:translate-x-full sm:duration-700">
                <div class="flex h-full flex-col overflow-y-auto bg-white py-6 shadow-xl">
                    <div class="px-4 sm:px-6">
                        <h2 id="drawer-title" class="text-base font-semibold text-gray-900">Assignment</h2>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('[command="show-modal"][commandfor="drawer"]').forEach(button => {
            button.addEventListener('click', () => {
                ...
                title.innerHTML = `<strong>Title :</strong> <br> ${assignment.title}`;
                rating.innerHTML = assignment.total_score ? `<strong >Score : <br> <span class='text-green-700'>${assignment.total_score} </span> </strong>` : '<strong>Score :</strong><br><span class="inline-block px-2 py-1 text-xs font-semibold text-amber-700 bg-amber-100 rounded">Not Evaluated</span>';
                feedback.innerHTML = assignment.feedback ? `<strong>Feedback :</strong> <br> ${assignment.feedback} ` : '<strong>Feedback :</strong>';
