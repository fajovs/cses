...
<main class="flex-1 overflow-y-auto px-30 py-6">
    ...
    <form method="POST" action=<?= base_url('/admin/users/parent/update') ?>>
        <div class="space-y-12">
            <input type="hidden" name="_method" value="PATCH" />
            <input type="hidden" name="user_id" value=<?= htmlspecialchars($users['user_id']) ?> />
            <div class="border-b border-gray-900/10 pb-12">
                <h2 class="text-base/7 font-semibold text-gray-900">Manage Parent</h2>
        ...
        <div class="mt-6 flex items-center justify-between gap-x-6">
            ...
            <div class="mt-6 flex items-center justify-end gap-x-6">
                ...
                <a href=<?= base_url("/admin/users/parents") ?> class="text-sm/6 font-semibold text-gray-900">
                    Cancel
                </a>
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <el-dialog-panel class="relative bg-white rounded-lg shadow-xl sm:w-full sm:max-w-lg">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse">
                    ...
                    <form method="POST" action=<?= base_url('/admin/users/parent/delete') ?>>
...
<script>
    ...
    if (programSelect && sectionsSelect) {
        ...
        if (existingProgramId) {
            programSelect.value = existingProgramId;
        }

        // Load sections initially for saved program
        if (existingProgramId) {
            loadSections(existingProgramId, existingSectionId);
        }

        // Load sections when program changes
        programSelect.addEventListener('change', () => {
            const selectedProgramId = programSelect.value;
            loadSections(selectedProgramId);
        });
    }

    async function loadSections(programId, selectedSectionId = null) {
        ...
        try {
            ...
            if (sections.length > 0) {
                let optionsHtml = '<option value="" disabled>Select Section</option>';
                sections.forEach(section => {
                    const selected = section.section_id === selectedSectionId ? 'selected' : '';
                    optionsHtml += `<option value="${section.section_id}" ${selected}>${section.section_name}</option>`;
                });
                sectionsSelect.innerHTML = optionsHtml;
            } else {
