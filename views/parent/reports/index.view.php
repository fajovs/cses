...
<main class="flex-1 overflow-y-auto px-30 py-6">
    ...
    <div class=" mx-auto max-w-7xl py-6 flex flex-col  sm:flex-row sm:items-end gap-4">
        ...
        <div>
            <label for="student_id" class="block text-sm font-medium text-gray-900 mb-3">Students</label>
            <select id="student_id" name="student_id" class="w-full appearance-none rounded-md bg-white py-1.5 pr-8 pl-3 text-base text-gray-900 outline-1 outline-gray-300 focus:outline-2 focus:outline-green-600 sm:text-sm/6" <?= empty($students) ? 'disabled' : 'required' ?>>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function getSubjects(studentId) {
            ...
            try {
                ...
                const response = await fetch('<?= base_url('/parent/reports/subjects') ?>?student=' + studentId);
                ...
                if (!data || subjects.length === 0) {
                    options = `<option selected disabled>No Subject Available</option>`;
                } else {
                    options = `<option value="" selected disabled>Select a Subject</option>`;
                ...
                <select id="subject_id" name="subject_id"
                    class="w-full appearance-none rounded-md bg-white py-1.5 pr-8 pl-3 text-base text-gray-900 outline-1 outline-gray-300 focus:outline-2 focus:outline-green-600 sm:text-sm/6"
                    ${subjects.length === 0 ? "disabled" : "required"}>
                    ${options}
                </select>
                `;
        ...
        async function generateReport(studentId, subjectId) {
            try {
                const response = await fetch(
                    `<?= base_url('/parent/reports/generate') ?>?student=${studentId}&subject=${subjectId}`
                );
                const data = await response.json();
